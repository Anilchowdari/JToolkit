 ...
 *-------------------------------------------------------------------------*/
/* This file is responsible for processing Dialog-Based Pathsend Requests in 32/64-bit PUT Environment
 * 32-bit
 * Supports TS/MP Pathsend APIs: _SERVERCLASS_DIALOG_BEGIN_, _SERVERCLASS_DIALOG_BEGINL_,
 *                               _SERVERCLASS_DIALOG_SEND_, _SERVERCLASS_DIALOG_SENDL_
 * Corresponding Pthreads APIs : _PUT_SERVERCLASS__DIALOG_BEGIN_, _PUT_SERVERCLASS_DIALOG_BEGINL_,
 *                               _PUT_SERVERCLASS_DIALOG_SEND_, _PUT_SERVERCLASS_DIALOG_SENDL_
 * 64-bit
 * Supports TS/MP Pathsend APIs: _SERVERCLASS_DIALOG_BEGINL64_, _SERVERCLASS_DIALOG_SENDL64_ (no small Pathsend support)
 * Corresponding Pthreads APIs: _PUT_SERVERCLASS_DIALOG_BEGINL_, _PUT_SERVERCLASS_DIALOG_SENDL_
 */
...
/*
	...
	Implemented on 28/09/2013. @Pabitra
*/
 ...
 *
 * JNI native method for TsmpDialog which makes call to PUT_SERVERCLASS_DIALOG_BEGIN_ of pthread.
 *
 * Formal Paramaters:
 *
...
//64-bit code
	...
	jbyte *msgbuf = NULL;
		...
		if (!put_serverclass_dialog_beginl_)
		{
			jRcv_throwUnsatisfiedLinkError(env, "Pthreads version mismatch. Unable to find symbol : PUT_SERVERCLASS_DIALOG_BEGINL_/PUT_SERVERCLASS_DIALOG_SENDL_\n");
			return;
		}
		 ...
		 * 2MB support
		 */
		fserror = put_serverclass_dialog_beginl_(&dlgId,
				(char *)pmonname,
		...
		{
			PUT_SERVERCLASS_SEND_INFO_(&pwerror, &fserror);
...
//32-bit code
	...
	if (pathSendMode == com_tandem_ext_util_PathsendModeType_TSMP_PATHSEND_32KB_LIMIT)
	{
		 ...
		 */		 
		fserror = PUT_SERVERCLASS_DIALOG_BEGIN_ (&dlgId,
			(char *)pmonname,
			(short)strlen(pmonname),
			(char *)scname,
			(short)strlen(scname),
 ...
 * Functional Description:
 *
 * JNI native method for TsmpDialog which makes call to PUT_SERVERCLASS_DIALOG_SEND_ of pthread. (In 64-bit,
 * it makes a call to PUT_SERVERCLASS_DIALOG_SENDL_ of pthread.
 *
 * Formal Paramaters:
 *
 *  env, The Java environment pointer                                   input
...
Java_com_tandem_tsmp_TsmpDialog_scDialogSend(JNIEnv *env, jobject obj,
	...
	jbyte *msgbuf = NULL;
		...
		*/
		...
		fserror = put_serverclass_dialog_sendl_(jdlgId,
						      (char *)msgbuf,
						      (char *)readbuf,
						      requestlength,
						      maxReplyLen,
...
//32-bit code starts
	...
	if (pathSendMode == com_tandem_ext_util_PathsendModeType_TSMP_PATHSEND_32KB_LIMIT)
	{
		 ...
		 */
		fserror = PUT_SERVERCLASS_DIALOG_SEND_(jdlgId,
			(char *)msgbuf,
			(short)requestlength,
			(short)maxReplyLen,
			&actualReplyLen,
 ...
 * Functional Description
 *
 * JNI native method for TsmpDialog which makes call to PUT_SERVERCLASS_DIALOG_ABORT_ of pthread. Remains same
 * for 32-bit as well as 64-bit Environments.
 * Formal Paramaters:
 *
 *  env, The Java environment pointer                       input
 *  obj, Java object representing TsmpDialog object         input
...
Java_com_tandem_tsmp_TsmpDialog_scDialogAbort(JNIEnv *env, jobject obj,
	...
	fserror = PUT_SERVERCLASS_DIALOG_ABORT_(jdlgId);
 ...
 * Functional Description:
 *
 * JNI native method for TsmpDialog which makes call to PUT_SERVERCLASS_DIALOG_END_ of pthread.
 * Remains same for 32-bit as well as 64-bit Environments
 * Formal Paramaters:
 *
 *  env, The Java environment pointer                       input
 *  obj, Java object representing TsmpDialog object         input
...
Java_com_tandem_tsmp_TsmpDialog_scDialogEnd(JNIEnv *env, jobject obj,
	...
	fserror = PUT_SERVERCLASS_DIALOG_END_(jdlgId);
