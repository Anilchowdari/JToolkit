...
struct oldreply_t {
	...
	unsigned int syncid; /* connection's request sync id if the reply
...
struct opener_t {
	...
	int node; /* node number of system running the opener,
...
struct rqstinfo_t {
	...
	synthesized. */
	unsigned int syncid;  /*connection's request 
...
struct myinfo_t {
	...
	unsigned int smm [2]; /* server's system message selection masks. */
 ...
 *                   int maxrplylen, unsigned int smm1, unsigned int smm2)
 *
 * Description:
...
int jpwy_open(short initopens, /* (in)  initial number of concurrent connec-
 ...
 locale support, respectively. All other bits ignored. */
		unsigned int smm1, unsigned int smm2) /* (in)  system messages that the server wants
...
unsigned long jpwy_setsmm(short mode, /* (in)  indicates how the current selections
 are to be updated.  See above. */
		unsigned int smm) /* (in)  selected system messages to be enabled
 or disabled.  See the *_SMM1 & *_SMM2
 #defines. */
{
	unsigned int oldsmm, newsmm;
 ...
 *
 * Syntax:
 *      OLDREPLY *reply_save (OPENFILE *open, unsigned int syncid,
...
static OLDREPLY *reply_save(OPENFILE *open, /* (in)  the open's table entry. */
		unsigned int syncid, /* (in)  the open's request sync id. */
 ...
 *
 * Syntax:
 *      OLDREPLY *reply_find (OPENFILE *open, unsigned int syncid)
 *
 * Description:
...
int jpwy_reply(char *msgbuf, /* (in)  reply message data to return to the
 ...
 with the reply. */
		...
		int *countReplied, /* (out) the actual number of bytes replied. */
		int pathSendMode) {
	...
	/* Solution 10-140512-1834 : Pthread lib: Jtoolkit receive reply method hangs */
 #ifndef __LP64
	if (pathSendMode
			==com_tandem_ext_util_PathsendModeType_TSMP_PATHSEND_32KB_LIMIT) {
		if (gmsgtag != -2 && put_REPLYX(msgbuf, msglen, &repcount, gmsgtag,
				error) != 0) {
			// put_replyxl failed get error info
			fierr = FILE_GETINFO_(JPWY_FNUM, &fserr, OMITREF, OMITSHORT,
					OMITREF, OMITREF, OMITREF);
			if (fierr != FEOK)
				fserr = fierr;
...
#endif
		if (gmsgtag != -2 && put_replyxl(msgbuf, msglen, countReplied, gmsgtag,
				error) != 0) {
			// put_replyxl failed get error info
			fierr = FILE_GETINFO_(JPWY_FNUM, &fserr, OMITREF, OMITSHORT,
					OMITREF, OMITREF, OMITREF);
 ...
 *
 * Syntax:
 *      int jpwy_initrecv (char *msgbuf, int buflen, int tag)
 *
 * Description:
...
int jpwy_initrecv(char *msgbuf, /* (in)  a message buffer to receive the request
 or system message. */
		...
		int *countRead, /* (out)  count Transferred */
		...
		int pathSendMode) /*Tsmp Version information */
{
	...
	err = put_RECEIVEREAD(JPWY_FNUM, msgbuf, buflen, countRead, -1,(short*)&msginfo->rcvinfo.rcvInfo, (short*)&msginfo->dlgflags);

	
	} else {
		err = put_receivereadl(JPWY_FNUM, msgbuf, buflen, countRead, -1,
				(short*)&msginfo->rcvinfo.rcvInfo2);
	}
	if (err < 0)
		err = (int)FEREQABANDONED;
		...
		int pathSendMode) /*Tsmp Version information */
{
	...
	if (GET_RECEIVEINFO_FIELD(rcvInfo, ReceiveType, pathSendMode)
		...
		if (fserr >= FEBADERR && (fserr != FEWRONGID || !sysmsg_selected(((SysMsg_Type *) msgbuf)->MsgType))) {
			...
			if (pathSendMode
				...
				replyerr=put_REPLYX(NULL, 0, NULL, GET_RECEIVEINFO_FIELD(rcvInfo,
						MessageTag, pathSendMode), fserr);
			else
			
				replyerr=put_replyxl(NULL, 0, NULL, GET_RECEIVEINFO_FIELD(rcvInfo,
						MessageTag, pathSendMode), fserr);
			return (FECONTINUE);
		}
	...
	if (sender != NULL && myinfo->recvdepth > 0) {
		...
		for(rqstx=sender->firstrqstx; rqstx!= ENDOFREQ;rqstx=request->nextrqstx)
		{
			...
			if (request->gmsgtag >= 0 && GET_RECEIVEINFO_FIELD(rcvInfo, SyncId,
				...
				if (pathSendMode
						==com_tandem_ext_util_PathsendModeType_TSMP_PATHSEND_32KB_LIMIT)
	 				replyerr=put_REPLYX(NULL, 0, NULL, request->gmsgtag, FEOK);
				else
					replyerr=put_replyxl(NULL, 0, NULL, request->gmsgtag, FEOK);
			...
			if (GET_RECEIVEINFO_FIELD(rcvInfo, SyncId, pathSendMode) != 0
				...
				if ((reply = reply_find(sender, GET_RECEIVEINFO_FIELD(rcvInfo,
					...
					if (pathSendMode
						...
						replyerr=put_REPLYX((char *) (reply + 1), reply->msglen, NULL,
								GET_RECEIVEINFO_FIELD(rcvInfo, MessageTag,
										pathSendMode), reply->error);
					else
				
						replyerr=put_replyxl((char *) (reply + 1), reply->msglen, NULL,
								GET_RECEIVEINFO_FIELD(rcvInfo, MessageTag,
										pathSendMode), reply->error);
				} else {
	...
	if (GET_RECEIVEINFO_FIELD(rcvInfo, ReceiveType, pathSendMode)
		...
		if (pathSendMode
			...
			replyerr=put_REPLYX((char *) rplybuf, rplylen, NULL, GET_RECEIVEINFO_FIELD(
					rcvInfo, MessageTag, pathSendMode), fserr);
		else
	
			replyerr=put_replyxl((char *) rplybuf, rplylen, NULL, GET_RECEIVEINFO_FIELD(
					rcvInfo, MessageTag, pathSendMode), fserr);
...
void jpwy_msginfo(short msgtag, /* (in)  identifies the message to retrieve. */
 ...
 specified message. */
	...
	if (myinfo->rqstinfo [msgtag].openx >= 0) {
		...
		SET_RECEIVEINFO_FIELD(rcvInfo, SyncId, (int)(request->syncid),
				pathSendMode) ;
...
void initialise_functionptr() {
	...
	put_initreceivel = (PUT_INITRECEIVEL) dlsym(dlopenhandle,
			"put_INITRECEIVEL");
	put_receivereadl=(PUT_RECEIVEREADL) dlsym(dlopenhandle, "put_RECEIVEREADL");
	put_replyxl=(PUT_REPLYXL) dlsym(dlopenhandle, "put_REPLYXL");
	put_serverclass_sendl_=(put_SERVERCLASS_SENDL_)dlsym(dlopenhandle,
			"PUT_SERVERCLASS_SENDL_");
	put_serverclass_dialog_beginl_=(put_SERVERCLASS_DIALOG_BEGINL_)dlsym(dlopenhandle,
			"PUT_SERVERCLASS_DIALOG_BEGINL_");
	put_serverclass_dialog_sendl_=(put_SERVERCLASS_DIALOG_SENDL_)dlsym(dlopenhandle,
