		...
		JNIEnv *env,jobject rcvFile)
{
	...
	pute = put_interrupt(fnum,PUT_ERROR);
...
)
{
	...
	union smm_t {
		long long whole_smm;
		struct smm_parts_t {
		/*Start of Soln. 10-130430-7220 */
			//unsigned long smm2;
			//unsigned long smm1;
			unsigned int smm2;
			unsigned int smm1;
		/*End of Soln: 10-130430-7220*/
		}smm_parts;
	}smm;
	...
	//regError = spt_INITRECEIVE(fnum,jreceiveDepth);// registration for narrow receive
		regError = put_INITRECEIVE(fnum,jreceiveDepth);
	else
	//regError = spt_initreceivel(fnum,jreceiveDepth);//registration for wide receive
		regError = put_initreceivel(fnum,jreceiveDepth);
...
(*env)->SetIntArrayRegion(env, jcountRead, 0 /*offset*/, 1 /*len*/, (int *)&buflen);
/*Changes end for 10-080226-0790 */
else
(*env)->SetIntArrayRegion(env, jcountRead, 0 /*offset*/, 1 /*len*/, (int *)&countTransferred);
