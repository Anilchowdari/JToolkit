...
static void fileIOHandler(const short filenum, const long tag,
	...
	spt_wakeup(filenum, tag, xferCount, error);
...
JNIEXPORT jobject JNICALL Java_com_hp_nonstop_io_process_impl_NSProcessChannel_jniOpen(
	 ...
	 */
	spt_status = spt_regFileIOHandler(fileNum, (spt_FileIOHandler_p)
			& fileIOHandler);

	if (spt_status != SPT_SUCCESS) {
		...
		/* Return Error code*/
		return createStatusObjectFrom(env, fileNum, spt_status);
	}
...
JNIEXPORT jobject JNICALL Java_com_hp_nonstop_io_process_impl_NSProcessChannel_jniRead(
	...
	if (ioEntry == NULL) {
		...
		if (spt_error || (error && (error != FE_CONTINUE)) || (ioEntry == NULL)) {
			/* We will be here in case of any FS error, Timeout or an Interrupt */

			logMsg(TRACE, "jniRead: spt_awaitio failed : fileNum=%d, ioTag=%d,"
					" error=%d, spt_error=%d.", fileNum, ioTag, error, spt_error);
			...
			if (spt_error) {
				return createStatusObjectFrom(env, fileNum, spt_error);
			}
