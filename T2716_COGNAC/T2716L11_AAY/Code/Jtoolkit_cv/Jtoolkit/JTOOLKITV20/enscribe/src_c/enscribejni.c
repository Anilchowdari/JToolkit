...

{
	...
	*fileName;
	...
	filenameLen = strlen(fileName);
		...
		error = FILE_ALTERLIST_ ((char _ptr32*)fileName1, filenameLen,
			(short _ptr32*)itemArray,
			numAttributes,
			 (short _ptr32*)attributeArray,
			attributeArrayLen,
			jpartOnly,
			(short _ptr32*)&errorItem);
   #else
		...
		/*in*/jboolean juse64BitPrimaryKeyValues)
{
	...
	*fileInfoArray;
	...
	put_error_t pute;
	int dummy;
	short typeInfo[5],iList[4],fnum,error;
	...
	pute = put_regFileIOHandler(fnum, &JEnscribe_callback);
...
JNIEXPORT jshort JNICALL Java_com_tandem_ext_enscribe_EnscribeFile_GfileRename(
		...
		/*in*/jboolean jfileNameIsOSS)
{
	jboolean bfileNameIsCopy,bnewFileNameIsCopy;
		...
		/*in*/short jsaveSize)
{
	...
	*saveBuf;
	...
	saveBuf = (*env)->GetByteArrayElements(env, jposData, &bposArrayIsCopy);
		...
		FILE_RESTOREPOSITION_( jfileNum, (short _ptr32*)saveBuf, jsaveSize);
	#else
		...
		/*out*/jarray jsaveSize)
{
	...
	*saveBuf;
	...
	saveCountArray = (*env)->GetShortArrayElements(env, jsaveSize, &bsaveCountArrayIsCopy);
		...
		#ifdef __LP64
			FILE_SAVEPOSITION_ ( jfileNum, (short _ptr32*)isaveBuf,(short)sizeof(isaveBuf), (short _ptr32*)&saveCount);
		#else
		...
		/*out*/jarray jlastParams)
{
	...
	#ifdef __LP64
		FILE_SETMODENOWAIT64_ (jfileNum, jfunction, , ,oldvalues, tag);
		lastParamsArray[0]=oldvalues[0];
		lastParamsArray[1]=oldvalues[1];
	#else
		SETMODENOWAIT (jfileNum, jfunction, , , lastParamsArray, tag);
	#endif
	}
	else
	if(jparam2Omitted){
	#ifdef __LP64
		FILE_SETMODENOWAIT64_ (jfileNum, jfunction, jparam1, ,oldvalues, tag);
		lastParamsArray[0]=oldvalues[0];
		lastParamsArray[1]=oldvalues[1];
	#else
		SETMODENOWAIT (jfileNum, jfunction, jparam1, , lastParamsArray,tag);
	#endif
	
	}
	else { 
	#ifdef __LP64
		FILE_SETMODENOWAIT64_ (jfileNum, jfunction, jparam1, jparam2, oldvalues, tag);
		lastParamsArray[0]=oldvalues[0];
		lastParamsArray[1]=oldvalues[1];
	#else
		SETMODENOWAIT (jfileNum, jfunction, jparam1, jparam2, lastParamsArray, tag);
	#endif
	}
		...
		/*out*/jarray jcountRead)
{
	...
	*combuf;
	...
	#ifdef __LP64
			WRITEREADX(jfileNum,(char _ptr32*)combuf, writebufLen, jmaxReadCount,,tag);
	#else
...

{
	...
	*errorArray;
	...
	errorArray = (*env)->GetShortArrayElements(env, jerrorArray, &berrorArrayIsCopy);
			...
			(char _ptr32*)thisFileName, 255,
			(short _ptr32*)&fileNameLen);
	#else
	error = FILENAME_FINDNEXT_(jsearchID,
			thisFileName, 255,
			&fileNameLen);
