...
{                                                                            
    ...
    */
    ...
    put_wakeup(filenum,tag,completionCount,fserror);
...
short JEnscribeWaitForCompletion(short fnum, long tag, int* countTransferred)
{
	...
	*countTransferred = 0;
	...
	pute = put_awaitio(fnum,tag,-1,countTransferred,&fserror,NULL);

	if(pute != PUT_SUCCESS) (void) CANCELREQ(fnum,tag);
...
short JEnscribeCancelReq(short fnum, long tag)
{
    ...
    */
    ...
    pute=put_wakeup(fnum,tag,(long)0,FEREQABANDONED);
